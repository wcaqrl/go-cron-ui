...
export class DefaultInterceptor implements HttpInterceptor {
  ...
  private refreshTokenRequest(): Observable<any> {
    ...
    return of();
    // return this.http.post(`/worker/list`, null, null, { headers: { refresh_token: model?.refresh_token || '' } });
  }
  ...
  private tryRefreshToken(ev: HttpResponseBase, req: HttpRequest<any>, next: HttpHandler): Observable<any> {
    ...
    if ([`/worker/list`].some((url) => req.url.includes(url))) {
      this.toLogin();
      ...
      case 200:
        ...
        if (ev instanceof HttpResponse) {
          ...
          if (body && body.errorCode !== 0) {
            ...
            return of(new HttpResponse({body: body.hasOwnProperty('data') ? body.data : body }));
          }
        }
        break;
      case 401:
